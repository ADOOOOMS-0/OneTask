...
import { Project, Task, Priority, Recurrence } from '../types';
...
const TaskBoard: React.FC<TaskBoardProps> = ({ project, onAddTask, onScheduleTask, onDeleteTask, onEditTask, onCompleteTask, isAutoRotationEnabled }) => {
  ...
  // Filter tasks based on schedule and recurrence
  const visibleTasks = project.tasks.filter(t => !t.scheduledDate || t.scheduledDate <= today);
  const scheduledAndRecurringTasks = project.tasks.filter(t => 
    (t.scheduledDate && t.scheduledDate > today) || 
    (t.recurrence && t.recurrence !== Recurrence.None)
  );
    ...
    <>
      ...
      <div className="mb-6 flex flex-nowrap gap-2 items-center justify-between sm:justify-start overflow-x-auto no-scrollbar pb-1">
        ...
        >
          ...
          <span className="hidden sm:inline">View Scheduled ({scheduledAndRecurringTasks.length})</span>
          <span className="sm:hidden">({scheduledAndRecurringTasks.length})</span>
        </button>
      </div>
      ...
      {isScheduledListOpen && (
        <ScheduledTasksModal 
            projects={[{ id: project.id, name: project.name, tasks: project.tasks }]}
            onClose={() => setScheduledListOpen(false)}
            onEditTask={(task) => { setScheduledListOpen(false); onEditTask(task); }}
        />
      )}
    </>
  );
